...
def create_patients_table(cur, conn):
    ...
    cur.execute("CREATE TABLE Patients (pet_id INTERGER PRIMARY KEY, name TEXT, species_id INTEGER, age INTEGER, cuteness INTEGER, aggressiveness INTEGER)")
    conn.commit()
...
def add_pets_from_json(filename, cur, conn):
    ...
    for pet in json_data:
        name = pet['name']
        age = pet['age']
        species = pet['species'] #must convert to id before submitting it to our db
        cuteness = pet['cuteness']
        aggressiveness = pet['aggressiveness']
        #get the id for species
        cur.execute("SELECT id FROM Species WHERE title = ?", (species,))
        species_id = cur.fetchone()[0] #getting the actual first value of tuple for species id
        conn.execute("INSERT INTO Patients (pet_id, name, species_id, age, cuteness, aggressiveness) VALUES (?,?,?,?,?,?)", (pet_id,name, age, species_id, cuteness, aggressiveness))
        pet_id += 1
...
def non_aggressive_pets(aggressiveness, cur, conn):
    ...
    non_aggressive_pet_list = cur.fetchall()
    non_aggressive_pet_list = [item[0] for item in non_aggressive_pet_list] #get the first value of tuple
